...
export default async function TransactionsPage() {
  ...
  const entries = (orders || []).map(o => {
    ...
    return { id: o.id, kind: "order" as const, title: o.order_number, subtitle: product, amount: -Number(o.total_amount), status: o.status, date: o.created_at, href: `/orders/${o.id}`, target: tx?.customer_no };
  });
  const walletEntries = (walletTx || []).map(t => ({ id: t.id, kind: "wallet" as const, title: t.type === "TOPUP" ? "Top Up Saldo" : t.type === "REFUND" ? "Refund" : "Transaksi Wallet", subtitle: t.reference_id ? `Ref: ${t.reference_id}` : "Saldo AIDIL STORE", amount: Number(t.amount), status: "SUCCESS", date: t.created_at }));
  ...
  return <div className="mx-auto max-w-6xl space-y-6 animate-page-in pb-10">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs font-black uppercase tracking-[.18em] text-amber-600">Pusat Aktivitas</p><h1 className="mt-1 text-3xl font-black tracking-tight">Riwayat Transaksi</h1><p className="mt-1 text-sm text-slate-500">Pantau PPOB, pesanan, pembayaran, dan perubahan saldo dari satu halaman.</p></div><Link href="/orders" className="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-center text-sm font-black text-slate-700 hover:bg-slate-50">Lihat Pesanan</Link></div>
