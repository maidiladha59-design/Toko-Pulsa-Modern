...
export async function GET(request: Request) {
  ...
  const { data: topups } = await admin.from('topups').select('id,status,amount,payment_amount,provider,provider_order_id,payment_method,expires_at,created_at').eq('provider','pakasir').not('provider_order_id','is',null).order('created_at',{ascending:false}).limit(150);
  for (const t of topups || []) {
    try {
      ...
      const provider = isPakasirConfigured() ? await getPakasirTransactionDetail(t.provider_order_id, Number(t.payment_amount || t.amount)) : null;
  ...
  const { data: orders } = await admin.from('orders').select('id').order('created_at',{ascending:false}).limit(150);
  for (const o of orders || []) {
    try { const {data}=await admin.rpc('reconcile_internal_financial_record',{p_source_type:'ORDER',p_source_id:o.id}); checked.push({type:'ORDER',id:o.id,state:data?.state}); }
