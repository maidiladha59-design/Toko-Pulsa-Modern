...
for (const f of ['package.json','.env.example','.gitignore','vercel.json','src/app/auth/callback/route.ts','src/app/api/payments/pakasir/webhook/route.ts','src/app/api/ppob/webhook/route.ts','src/app/api/ppob/cron/route.ts']) {
  exists(f) ? pass(`file:${f}`) : fail(`file:${f}`,'missing');
}

// 2. Environment contract
const env = read('.env.example');
const required = ['NEXT_PUBLIC_SUPABASE_URL','NEXT_PUBLIC_SUPABASE_ANON_KEY','SUPABASE_SERVICE_ROLE_KEY','PAKASIR_PROJECT','PAKASIR_API_KEY','DIGIFLAZZ_USERNAME','DIGIFLAZZ_API_KEY','DIGIFLAZZ_WEBHOOK_SECRET','INTERNAL_CRON_SECRET','CRON_SECRET'];
...
if (env.includes('PAKASIR_SANDBOX=true')) pass('pakasir-default','Example environment defaults to sandbox.');
...
const pakWebhook = read('src/app/api/payments/pakasir/webhook/route.ts');
...
pakWebhook.includes('verify') || pakWebhook.includes('Pakasir') ? pass('pakasir-webhook-handler','Pakasir webhook handler present.') : warn('pakasir-webhook-handler','Could not statically prove callback verification; perform live sandbox test.');
...
const pak = read('src/lib/pakasir.ts');
pak.includes('pakasir.com') ? pass('pakasir-client','Pakasir integration module present.') : warn('pakasir-client','Pakasir hostname not found statically.');
...
if (live) {
  ...
  if (liveKeys.length) fail('live-env','Missing environment variables: '+liveKeys.join(', '));
    ...
    if (process.env.PAKASIR_SANDBOX === 'false') pass('pakasir-live-mode','Pakasir sandbox disabled.'); else warn('pakasir-live-mode','Pakasir is still in sandbox.');
