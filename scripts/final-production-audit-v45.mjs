...
for(const k of ['NEXT_PUBLIC_SUPABASE_URL','NEXT_PUBLIC_SUPABASE_ANON_KEY','SUPABASE_SERVICE_ROLE_KEY','NEXT_PUBLIC_SITE_URL','PAKASIR_PROJECT','PAKASIR_API_KEY','DIGIFLAZZ_USERNAME','DIGIFLAZZ_API_KEY','DIGIFLAZZ_WEBHOOK_SECRET','CRON_SECRET']) envExample.includes(`${k}=`)?ok(`env:${k}`,'documented'):fail(`env:${k}`,'not documented');
...
for(const r of ['src/app/api/ppob/webhook/route.ts','src/app/api/payments/pakasir/webhook/route.ts','src/app/api/ppob/cron/route.ts','src/app/api/admin/monitoring/route.ts']){if(!exists(r))fail('critical-route',r);else ok(`route:${r}`,'present')}
...
const pak=exists('src/app/api/payments/pakasir/webhook/route.ts')?read('src/app/api/payments/pakasir/webhook/route.ts'):''; if(pak && !/transaction|amount|order_id|status/i.test(pak))warn('pakasir-webhook','static audit could not confirm provider detail fields; verify live integration.');
